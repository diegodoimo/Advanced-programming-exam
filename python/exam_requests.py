...
class PostcardList:
    ...
    def writeFile(self, filename):
        ...
        with open(self._file, 'w') as f:
            for i in range(getNumberOfPostcards()):
                print(self._postcards[i], file = f)
    ...
    def updateLists(self, filename):
        ...
        with open(self._file, 'r') as f:
            update = f.readlines()
        self._postcards.append(update)

    def getNumberOfPostcards(self):
        return len(self._postcards)

    def parsePostcards(self):
        for index, line in enumerate(self._postcards):
            tmp = re.split(';|:', line)

            if tmp[1] in self._date:
                self._date[tmp[1]].append(index)
            else:
                self._date[tmp[1]] = [index]

            if tmp[3] in self._from:
                self._from[tmp[3]].append(index)
            else:
                self._from[tmp[3]] = [index]

            if tmp[5] in self._to:
                self._to[tmp[5]].append(index)
            else:
                self._to[tmp[5]] = [index]
        ...
        for i in self._date:
            ...
            if (date >= date_range[0] and date<= date_range[1]):
                for j in self._date[i]:
                    postcards.append(self._postcards[j])
        return postcards
